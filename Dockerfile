...
#     -t pi-sandbox:latest .
#
# Run (override any field at runtime — required, no real defaults baked in):
#   docker run --rm -it \
...
# ---------------------------------------------------------------------------
# MCP bridge (mcp mode)
#   Node HTTP server exposing pi as MCP tools (Hermes -> pi_sandbox_mcp_*).
#   Runs as the non-root ${PI_USER_NAME}; port 3103, bearer-key auth.
# ---------------------------------------------------------------------------
COPY --chown=${PI_UID}:${PI_GID} mcp/ /opt/pi-mcp/
RUN cd /opt/pi-mcp && npm ci --omit=dev --ignore-scripts
...
# Provider config
# NOTE: placeholders only — override at runtime (docker run -e / compose /
# .env). The entrypoint refuses to start if these are still placeholders.
ENV PI_LLM_BASE_URL=http://your-llm-host:8000/v1
ENV PI_LLM_API=openai-completions
ENV PI_LLM_API_KEY=***

# Model config
ENV PI_LLM_MODEL_ID=your-org/your-model
ENV PI_LLM_MODEL_NAME="your-org/your-model (example)"
